...
class Ui_Form(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Form):
        ...
        self.cb2.addItems(configUtils.group_range)
        self.cb2.setCurrentIndex(0)
        self.cb2.currentIndexChanged.connect(lambda: configUtils.set_range(self.cb2.currentIndex()))
    ...
    def stat(self, export):
        ...
        try:
            ...
            jql = "worklogDate >= '%(from)s' and worklogDate <= '%(to)s' and cf[11907]" \
                  " is not EMPTY" % {'from': fromDate,'to': toDate}
            jira = configUtils.jira_client()
            # if not hasattr(self, 'worklogs') or not self.worklogs:
            if True:
                issues = jira.search_issues(jql, maxResults=-1, fields=['customfield_11907','customfield_12000', 'summary', 'project', 'worklog'])
                worklogs = jiraUtils.get_worklogs(issues, '项目', diff, self.dateEdit.dateTime(), self.dateEdit2.dateTime())
                issue = jira.issue('APITEAM-9246')
                regions = json.loads(issue.fields.description.replace(' ', ''))
                for w in worklogs:
                    w['区域'] = '其它'
                    for k, v in regions.items():
                        if v.__contains__(w['项目']):
                            w['区域'] = k
                self.worklogs = worklogs
            if self.worklogs:
                df = pd.DataFrame(self.worklogs)
                gr = ['区域', '项目', '产品', '部门', '姓名', '工作事项']
                cgr = configUtils.group_range[self.cb2.currentIndex()]
                for index, item in enumerate(gr):
                    if cgr.__contains__(item):
                        ...
                        if export:
                            ...
                            df.groupby(['区域']).agg('sum').to_excel(writer, "区域汇总")
                            # df.plot.pie(subplots=True).to_excel(writer, "区域汇总")
                            df.groupby(['项目']).agg('sum').to_excel(writer, "项目汇总")
