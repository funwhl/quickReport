...
class Ui_Form(QtWidgets.QMainWindow):
    ...
    def stat(self, export):
        ...
        try:
            ...
            jql = "updated >=%(name)sd and worklogDate >= %(name)sd and project = %(pName)s" % {
                'name': diff,
                'pName': list(configUtils.projects.keys())[configUtils.group_index()]}
            jira = configUtils.jira_client()
            if not hasattr(self, 'worklogs') or not self.worklogs:
                issues = jira.search_issues(jql, maxResults=-1, fields=['summary'])
                self.worklogs = jiraUtils.get_worklogs(issues, '组内', diff, None, None)
            if self.worklogs:
                df = pd.DataFrame(self.worklogs)
                self.textEdit.setHtml(df.groupby(['姓名', '工作事项']).agg('sum').to_html())
                if export:
                    ...
                    df.groupby(['姓名', '工作事项']).agg('sum').to_excel(writer, "组内工作汇总")
                    writer.save()
